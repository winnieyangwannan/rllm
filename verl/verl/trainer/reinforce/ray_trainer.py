...
from verl.trainer.reinforce import core_algos
...
def compute_data_metrics(batch):
    ...
    metrics = {
        ...
        'actor/rewards/min': torch.min(sequence_reward).detach().item(),
        # adv
        'actor/advantages/mean': masked_mean(advantages, response_mask).detach().item(),
        'actor/advantages/max': torch.max(advantages[response_mask.bool()]).detach().item(),
        'actor/advantages/min': torch.min(advantages[response_mask.bool()]).detach().item(),
        # response length
...
class RayReinforceTrainer(object):
    """
    ...
    def _compute_advantage(self, data):
        ...
        if adv_estimator == "grpo":
            ...
            # Expand rewards to match token dimension and apply mask
            expanded_rewards = normalized_rewards.unsqueeze(-1).expand(-1, response_length)
            masked_advantages = expanded_rewards * response_mask
            data.batch['advantages'] = masked_advantages
        else:
            raise NotImplementedError
