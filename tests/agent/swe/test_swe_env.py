...
from rllm.environments.swe.swe import BatchSWEEnv, SWEEnv, R2E_ENV_IDS
...
def batch_swe_env():
    ...
    env = BatchSWEEnv(
        ...
        split=TEST_SPLIT,
        seeds=seeds,
        truncate_dataset_size=TRUNCATE_SIZE
    )
    yield env
...
def test_step_subset_envs(batch_swe_env: BatchSWEEnv):
    ...
    observations, rewards, terminateds, truncateds, infos = batch_swe_env.step(
        actions, env_idxs=env_idxs_to_step
    )
...
def test_from_json(mock_swe_env, mock_batch_init):
    ...
    extra_infos = [
        {"seed": 10, "some_other_key": "value1"},
        {"seed": 20, "another_key": "value2"}
    ]
