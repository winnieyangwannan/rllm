...
def test_eval_with_proxy_mode(runner, tmp_rllm_home, mock_dataset):
    ...
    with patch("rllm.experimental.eval.config.load_config", return_value=config), \
         patch("rllm.experimental.eval.proxy.EvalProxyManager", return_value=mock_pm), \
         patch("rllm.experimental.cli.eval._run_eval") as mock_run:
...
def test_eval_base_url_skips_proxy(runner, tmp_rllm_home, mock_dataset):
    ...
    with patch("rllm.experimental.eval.proxy.EvalProxyManager") as mock_pm_cls, \
         patch("rllm.experimental.cli.eval.load_agent", return_value=mock_agent), \
         patch("rllm.experimental.cli.eval.resolve_evaluator_from_catalog", return_value=_MockEvaluator()):
        result = runner.invoke(cli, [
            "eval", "test_math",
            "--agent", "math",
...
def test_eval_with_mock_agent(runner, tmp_rllm_home, mock_dataset):
    ...
    with patch("rllm.experimental.cli.eval.load_agent", return_value=mock_agent), \
         patch("rllm.experimental.cli.eval.resolve_evaluator_from_catalog", return_value=_MockEvaluator()):
        result = runner.invoke(cli, [
            "eval", "test_math",
            "--agent", "math",
...
def test_eval_with_explicit_evaluator(runner, tmp_rllm_home, mock_dataset):
    ...
    with patch("rllm.experimental.cli.eval.load_agent", return_value=mock_agent), \
         patch("rllm.experimental.cli.eval.load_evaluator", return_value=_MockEvaluator()) as mock_load_eval:
        result = runner.invoke(cli, [
            "eval", "test_math",
            "--agent", "math",
