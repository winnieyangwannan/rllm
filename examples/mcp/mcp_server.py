...
logging.basicConfig(level=logging.DEBUG, format="%(asctime)s - %(levelname)s - %(message)s", filename="mcp_server.log", filemode="w")
...
def execute_python_locally(code: str) -> str:
    try:
        ...
        result = subprocess.run([sys.executable, temp_filename], capture_output=True, text=True, timeout=10)
...
async def firecrawl_tool(url: str) -> str:
    result = await tool_registry.instantiate("firecrawl")(url, use_async=True)
    return format_tool_output(result)


@mcp.tool()
async def tavily_tool(query: str, search_depth: str = "basic") -> str:
    result = await tool_registry.instantiate("tavily")(query=query, search_depth=search_depth, use_async=True)
    return format_tool_output(result)
