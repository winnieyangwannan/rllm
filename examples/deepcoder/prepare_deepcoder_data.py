...
def prepare_deepcoder_data(train_size: int = 24000, test_size: int = 500):
    ...
    def process_lcb_example(example, idx, split="train"):
        ...
        for test in test_cases:
            formatted_test = {"input": test.get("input", ""), "output": test.get("output", ""), "metadata": {"testtype": "functional"}}
        ...
        return {"question": question, "ground_truth": formatted_tests, "data_source": "livecodebench", "uid": f"lcb_{split}_{example['question_id']}", "split": split, "index": idx, "difficulty": example["difficulty"], "question_id": example["question_id"], "contest_id": example["contest_id"], "contest_date": str(example["contest_date"]), "starter_code": example.get("starter_code", "")}
