...
class LocalRetriever:
    ...
    def _load_indices(self):
        ...
        if sparse_index_file.exists():
            with open(sparse_index_file) as f:
    ...
    def dense_search(self, query: str, k: int = 10) -> list[dict[str, Any]]:
        ...
        for score, idx in zip(scores[0], indices[0], strict=False):
            if idx < len(self.corpus):
                results.append({"content": self.corpus[idx], "score": float(score), "type": "dense"})
    ...
    def sparse_search(self, query: str, k: int = 10) -> list[dict[str, Any]]:
        ...
        for idx in top_indices:
            if idx < len(self.corpus) and scores[idx] > 0:
                results.append({"content": self.corpus[idx], "score": float(scores[idx]), "type": "sparse"})
    ...
    def hybrid_search(self, query: str, k: int = 10, dense_weight: float = 0.7) -> list[dict[str, Any]]:
        ...
        for content, score in sorted_results[:k]:
            results.append({"content": content, "score": score, "type": "hybrid"})
...
def health_check():
    ...
    return jsonify({"status": "healthy", "corpus_size": len(retriever.corpus) if retriever else 0, "dense_index_loaded": retriever.dense_index is not None if retriever else False, "sparse_index_loaded": retriever.sparse_index is not None if retriever else False})
...
def retrieve():
    ...
    try:
        ...
        # Perform search
        results = retriever.search(query=query, method=method, k=k, dense_weight=dense_weight)

        # Format results for the tool
        formatted_results = []
        for i, result in enumerate(results, 1):
            formatted_results.append({"id": f"doc_{i}", "content": result["content"], "score": result["score"]})
