...
def prepare_search_data(train_size=3000, test_size=100):
    ...
    def process_hotpot_example(example, idx, split):
        return {"question": example["question"], "ground_truth": example["answer"], "data_source": "hotpotqa", "uid": f"hotpot_{example.get('id', idx)}", "split": split, "index": idx, "question_type": example.get("type", "bridge"), "level": example.get("level", "medium")}

    def process_nq_example(example, idx, split):
        ground_truth = example["answer"][:200] + "..." if len(example["answer"]) > 200 else example["answer"]
        return {"question": example["query"], "ground_truth": ground_truth, "data_source": "natural_questions", "uid": f"nq_{idx}", "split": split, "index": idx, "question_type": "factual", "level": "easy"}
    ...
    DatasetRegistry.register_dataset("hotpotqa_combined", hotpot_val_processed, "test")
