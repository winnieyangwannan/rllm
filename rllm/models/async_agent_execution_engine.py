...
from rllm.models.scheduler import Scheduler
import torch
import numpy as np
import time
import os
...
class AsyncAgentExecutionEngine(AgentExecutionEngine):
    def __init__(
        ...
        **kwargs,
        ...
        self.scheduler = Scheduler(rollout_engine=rollout_engine)
    ...
    async def get_action_async(self, trajectory, agent, application_id, **kwargs):
        if self.engine_name == "openai":
            ...
            return await self._get_action_verl_async(trajectory, agent, application_id, **kwargs)
        else:
    ...
    async def run_agent_trajectory(self, idx, application_id, seed=0, mode="Text", **kwargs):
        ...
        for _ in range(self.max_episode_len):
            ...
            if response_len + len(assistant_msg_tokens) + len(env_msg_tokens) + self.max_prompt_length >= self.max_trajectory_length:
                # Truncation length
                truncation_length = self.max_trajectory_length - self.max_prompt_length - response_len
