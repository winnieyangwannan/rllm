...
class VerlEngine(RolloutEngine):
    ...
    async def get_model_response(self, messages: list[dict], **kwargs) -> ModelOutput:
        ...
        if max_tokens is not None and len(response_ids) >= max_tokens:
            finish_reason = "length"
            response_ids = response_ids[:max_tokens]

        response_text = self.tokenizer.decode(response_ids, return_special_tokens=False)

        tool_calls = None
        if self.tool_parser is not None:
            tool_calls = self.tool_parser.parse(response_text)

        return ModelOutput(text=response_text, tool_calls=tool_calls, finish_reason=finish_reason, completion_tokens=len(response_ids))
