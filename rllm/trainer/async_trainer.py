...
class RayPPOAsyncTrainer(RayPPOTrainer):
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            # Split test_batch into smaller chunks for parallel processing
            batch_chunks = list(split(test_batch, validate_wg.world_size))
            
            # Track completed requests and their results
            completed_results = []
            pending_refs = {}  # Maps object_ref to chunk index
            
            # Initial filling of the queue
            queue_count = 0
            
            for i in range(min(validate_wg.world_size, len(batch_chunks))):
                worker_idx = i % validate_wg.world_size
                obj_ref = validate_wg.execute_worker_async(
                    worker_idx=worker_idx, 
                    method_name='rollout_generate', 
                    prompts=batch_chunks[i]
                )
            ...
            while pending_refs:
                ...
                chunk_idx = pending_refs.pop(done_ref)
                completed_results.append((chunk_idx, result))
                print(f'completed_results: {result}')
                print("decoded: ", [self.tokenizer.decode(ids, skip_special_tokens=True) for ids in result.batch["responses"]])
                
                # Add a new task if there are more chunks
                next_idx = queue_count
                if next_idx < len(batch_chunks):
                    worker_idx = next_idx % validate_wg.world_size
                    obj_ref = validate_wg.execute_worker_async(
                        worker_idx=worker_idx, 
                        method_name='rollout_generate', 
                        prompts=batch_chunks[next_idx]
                    )
                    pending_refs[obj_ref] = next_idx
                    queue_count += 1
            
            # Sort results by chunk index and process them
            completed_results.sort(key=lambda x: x[0])
            test_output_items = [result for _, result in completed_results]
            
            # Combine results
            test_batch = DataProto.concat(test_output_items)
